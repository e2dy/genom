...
var (
	...
	netFlag     = flag.Uint64("network", 0, "Network ID to use for the Genom protocol")
	...
	logFlag    = flag.Int("loglevel", 3, "Log level to use for Genom and the faucet")
)
...
type request struct {
	...
	Account common.Address     `json:"account"` // Genom address being funded
	Time    time.Time          `json:"time"`    // Timestamp when the request was accepted
	Tx      *types.Transaction `json:"tx"`      // Transaction funding the account
}

// faucet represents a crypto faucet backed by an Genom light client.
type faucet struct {
	config *params.ChainConfig // Chain configurations for signing
	stack  *node.Node          // Genom protocol stack
	client *ethclient.Client   // Client connection to the Genom chain
...
func newFaucet(genesis *core.Genesis, port int, enodes []*discv5.Node, network uint64, stats string, ks *keystore.KeyStore, index []byte) (*faucet, error) {
	...
	// Assemble the Genom light client protocol
...
// close terminates the Genom connection and tears down the faucet.
...
func (f *faucet) apiHandler(conn *websocket.Conn) {
	...
	for {
		...
		// Retrieve the Genom address to fund, the requesting user and a profile picture
		var (
			username string
			avatar   string
...
// the username, avatar URL and Genom address to fund on success.
func authGitHub(url string) (string, string, common.Address, error) {
	...
	// Iterate over all the files and look for Genom addresses
	...
	if address == (common.Address{}) {
		return "", "", common.Address{}, errors.New("No Genom address found to fund")
	}
...
func authTwitter(url string) (string, string, common.Address, error) {
	...
	// scrape it for the Genom address and profile URL.
...
// returning the username, avatar URL and Genom address to fund on success.
...
// authNoAuth tries to interpret a faucet request as a plain Genom address,
