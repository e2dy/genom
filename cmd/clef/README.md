...
This setup allows a DApp to connect to a remote Genom node and send transactions that are locally signed. This can
help in situations when a DApp is connected to a remote node because a local Genom node is not available, not
synchronised with the chain or a particular Genom node that has no built-in (or limited) account management.
