...
// nodeDockerfile is the Dockerfile required to run an Genom node.
var nodeDockerfile = `
...
// an Genom node (bootnode or miner for now).
var nodeComposefile = `
version: '2'
services:
  {{.Type}}:
...
// deployNode deploys a new Genom node container to a remote machine via SSH,
